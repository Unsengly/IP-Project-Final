...
import styles from "./AdminTeacherPage.module.css";
...
function AdminTeacherPage() {
  ...
  const onFinish = async (item) => {
    ...
    const res = await request("teacher", method, body);
    if (res?.error) {
  ...
  return (
    <MainAdminPage loading={loading}>
      <div className={styles.contianFilter}>
        <Space>
          <div>
            <p className="txtTitle">Teacher</p>
          </div>
      ...
      />
      <Modal maskClosable={false} open={visible} onCancel={onCloseForm} title={formTeacher.getFieldValue("Id") ? "Edit Teacher" : "New Teacher"} footer={null}>
